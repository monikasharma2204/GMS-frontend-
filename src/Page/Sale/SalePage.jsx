...
import { useRecoilState, useResetRecoilState, useRecoilValueLoadable } from "recoil";
...
const SalePage = () => {
  ...
  useTransactionNavigationGuard(shouldShowUnsavedDialog, resetSaleState);
  ...
  const onHandleSave = async () => {
    ...
    let accountId = memoInfo?.account?._id;

    if (!accountId && memoInfo?.account && customerList.length > 0) {
      const accountName = typeof memoInfo.account === 'object' ? memoInfo.account.label : memoInfo.account;
      if (accountName) {
        const matched = customerList.find(c => c.label === accountName || c.vendor_code_name === accountName);
        if (matched) accountId = matched._id;
      }
    }
    ...
    if ((memoInfo?.id || memoInfo?._id) && !memoInfo?.from_reserve && !memoInfo?.isReserveEdit) {
      method = "PUT";
  ...
  const handleCancelEdit = () => {
    ...
    if (fsmState === "initial") {
      if (hasUnsavedData()) {
        ...
        setPendingSelection("CANCEL_INITIAL");
        return;
      } else {
  ...
  const handleExportExcel = () => {
    try {
      ...
      const totalLabourAmt = rows.reduce((a, r) => a + (Number(r.labour_price) || 0), 0);

      /* =========================
         SUMMARY DATA
      ========================= */
      const summaryHeaders = [
        "Doc Date", "Due Date", "Sale No.", "Account", "Ref 1", "Ref 2",
        "Currency", "Exc Rate", "Pcs", "Weight", "Labour Amt", "SubTotal",
      ...
      const summaryValues = [
        ...
        memoInfo?.invoice_no || "",
        (memoInfo?.account?.label || (typeof memoInfo?.account === 'string' ? memoInfo.account : "")) || "",
        ref1 || "",
        ref2 || "",
      ...
      ========================= */
      const itemHeaders = [
        "Location", "Type", "Lot", "Stone Code", "Stone", "Shape", "Size", "Color",
        "Cutting", "Quality", "Clarity", "Cer Type", "Cer No.", "Pcs", "Weight",
        "Price", "Unit", "Amount", "Labour", "Labour Price", "Remark"
      ];
      ...
      const itemRows = rows.map(row => ([
        ...
        row.labour || "",
        Number(row.labour_price || 0),
        row.remark || ""
      ]));

      exportTransactionToExcel({
        filename: memoInfo?.invoice_no || "Sale",
        sheetName: "Sale",
        summaryHeaders,
        summaryValues,
        itemHeaders,
  ...
  const handleSaveSuccess = () => {
    ...
    if (memoInfo?.id) {
      ...
      const originalDataForEdit = {
        ...
        from_reserve: false,
        isReserveEdit: false,
        reserve_id: memoInfo?.reserve_id || null,
      };
  ...
  const handleEdit = (item) => {
    ...
    const accountIdFromItem = typeof item.account === 'object' ? item.account._id : undefined;

    let accountObj = null;
    if (customerList.length > 0) {
      accountObj = customerList.find(c =>
        (accountIdFromItem && c._id === accountIdFromItem) ||
        (accountName && (c.label === accountName || c.vendor_code_name === accountName))
      );
    }

    if (!accountObj) {
      accountObj = typeof item.account === 'object' ? {
        ...item.account,
        label: accountName,
        code: accountCode,
        _id: accountIdFromItem
      } : {
        label: item.account,
  ...
  const handleReturnReserveEdit = (reserveItem) => {
    ...
    // Set FSM state to "initial" mode when Reserve data is loaded
    setFsmState("initial");
    setEditMemoStatus(true);

    const reserveAccountName = typeof reserveItem.account === 'object' ? (reserveItem.account.vendor_code_name || reserveItem.account.label || "") : reserveItem.account;
    const reserveAccountCode = typeof reserveItem.account === 'object' ? (reserveItem.account.code || reserveItem.vendor_code_id || "") : reserveItem.vendor_code_id;
    const reserveAccountIdFromItem = typeof reserveItem.account === 'object' ? reserveItem.account._id : undefined;

    let reserveAccountObj = null;
    if (customerList.length > 0) {
      reserveAccountObj = customerList.find(c =>
        (reserveAccountIdFromItem && c._id === reserveAccountIdFromItem) ||
        (reserveAccountName && (c.label === reserveAccountName || c.vendor_code_name === reserveAccountName))
      );
    }

    if (!reserveAccountObj) {
      reserveAccountObj = typeof reserveItem.account === 'object' ? {
        ...reserveItem.account,
        label: reserveAccountName,
        code: reserveAccountCode,
        _id: reserveAccountIdFromItem
      } : {
        label: reserveItem.account,
        code: reserveItem.vendor_code_id
      };
    }
    ...
    setMemoInfo({
      ...
      from_reserve: true,
      isReserveEdit: true // Flag to indicate Reserve data is loaded
    });
      ...
      <Box sx={{ marginLeft: "222px", Height: "100vh ", paddingBottom: "130px" }}>
        ...
        <ConfirmCancelDialog
          ...
          onClose={(confirmed) => {
            ...
            if (confirmed) {
              if (pendingSelection === "CANCEL_INITIAL") {
