...
const QuotationsOrder = () => {
  ...
  useEffect(() => {
    if (memoInfo?.id || memoInfo?.isDayBookEdit) {
      ...
      setOriginalData((prevOriginalData) => {
        if (prevOriginalData && prevOriginalData.items && Array.isArray(prevOriginalData.items) && prevOriginalData.items.length > 0) {
  ...
  const restoreFromOriginalData = (data) => {
    ...
    if (rows && Array.isArray(rows)) {
      rows.forEach((row) => {
        if (row.image_preview && typeof row.image_preview === "string" && row.image_preview.startsWith("blob:")) {
  ...
  const handleSaveSuccess = (savedData = null) => {
    ...
    const recordId = savedData?._id || savedData?.id || memoInfo?.id;
    ...
    const savedItems = savedData?.items && Array.isArray(savedData.items) ? savedData.items : rows;


    if (savedData?.items && Array.isArray(savedData.items) && savedData.items.length > 0) {
      const formattedItems = savedData.items.map((el) => {
        let imageUrl = null;
        let imagePath = null;
        if (el.image) {
    ...
    if (recordId || savedItems.length > 0) {
      const completeData = {
        ...formData,
        _id: recordId,
  ...
  const shouldShowUnsavedDialog = useMemo(() => {
    ...
    return hasUnsavedQuotationData();
  }, [fsmState, rows]);

  useTransactionNavigationGuard(shouldShowUnsavedDialog, resetQuotationState);
  ...
  const onHandleSave = async () => {
    ...
    if (!account?.label || !account?.code || !account?._id) {
      invalidList.push({
        row: "Header",
        list: ["Account"],
      });
  ...
  const handleEditQuotation = (item) => {
    ...
    if (
      ...
      if (matchedAccount) {
        setAccountRaw(matchedAccount);
        allInvoiceAddresses = matchedAccount.invoiceAddress || [];
        allShippingAddresses = matchedAccount.shippingAddress || [];
      } else {

        setAccountRaw({
          label: typeof item.account === 'object' ? item.account?.vendor_code_name : item.account,
          code: item.vendor_code_id,
          _id: typeof item.account === 'object' ? item.account?._id : item.account,
        });
      }
    } else {
  ...
  const handleExportExcel = () => {
    try {
      ...
      const summaryHeaders = [
        "Doc Date", "Due Date", "QT No.", "Account", "Ref 1", "Ref 2",
      ...
      exportTransactionToExcel({
        filename: memoInfo?.invoice_no || "Quotation",
        sheetName: "Quotation",
        summaryHeaders,
        summaryValues,
        itemHeaders,
