      ...
      if ((!finalData._id || finalData._id === undefined) && finalData.code) {
        try {
          const response = await axios.get(
            `${API_URL}/master?master_type=master_stone_clarity&code=${encodeURIComponent(finalData.code)}`
          );
    ...
    if (!savedId && formData?.code) {
      ...
      for (let attempt = 0; attempt < 3; attempt++) {
        try {
          ...
          const response = await axios.get(
            `${API_URL}/master?master_type=master_stone_clarity&code=${encodeURIComponent(formData.code)}`
          );
