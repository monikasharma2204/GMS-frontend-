      ...
      if ((!finalData._id || finalData._id === undefined) && finalData.code) {
        try {
          const response = await axios.get(
            `${API_URL}/master?master_type=master_stone_color&code=${encodeURIComponent(finalData.code)}`
          );
          ...
          // Continue with data as-is if fetch fails
        }
      }
    ...
    if (!savedId && formData?.code) {
      ...
      for (let attempt = 0; attempt < 3; attempt++) {
        try {
          ...
          const response = await axios.get(
            `${API_URL}/master?master_type=master_stone_color&code=${encodeURIComponent(formData.code)}`
          );
