...
const MemoInOrder = () => {
  ...
  const restoreFromOriginalData = (data) => {
    ...
    if (data.doc_date) setDocDate(parseBackendDate(data.doc_date));
    if (data.due_date) setDueDate(parseBackendDate(data.due_date));
  ...
  useTransactionNavigationGuard(shouldShowUnsavedDialog, resetPurchaseState);
  ...
  useEffect(() => {
    ...
    const isOnPurchasePage = location.pathname === "/purchase-order/purchase";
    const hasNoRows = !rows || rows.length === 0;
    const isNotDayBookEdit = !memoInfo?.isDayBookEdit;
    const isInSavedState = fsmState === "saved";


    if (isOnPurchasePage && hasNoRows && isNotDayBookEdit && isInSavedState) {
  ...
  const onHandleSave = async (skipReload = false) => {
    ...
    if (!accountId) {
      setWarningText("Invalid Account Selected");
      setIsOpenModalConfrim(false);
      setIsOpenModalWarning(true);
      return;
    }
  ...
  const handleExportExcel = () => {
    try {
      ...
      const summaryHeaders = [
        "Doc Date", "Due Date", "Purchase No.", "Account", "Ref 1", "Ref 2",
      ...
      exportTransactionToExcel({
        filename: memoInfo?.invoice_no || "Purchase",
        sheetName: "Purchase",
        summaryHeaders,
        summaryValues,
        itemHeaders,
  ...
  const handleEdit = (item) => {
    ...
    const formattedItems = item.items.map((el) => ({
      ...
      type: (el.stock_id || el.isFromMemoPending) ? "Cons." : (el.type || el.stone_type || ""),
