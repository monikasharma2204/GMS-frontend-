...
import {showConfirmModalState,confirmValueState,successModalState,successValueState,failedModalState,failedValueState,displayConfirmSaveDialogState,suscessDialogState,unSuscessDialogState} from "recoil/DialogState"
import { useLocation,useNavigate } from "react-router-dom";
import { useRecoilState } from "recoil";
import apiRequest from "helpers/apiHelper";
import {payloadUrlState,payloadMethodState,payloadDataState} from "recoil/state/CommonState"

const ConfirmSaveDialog = (props)  => {
const navigate = useNavigate()

const [payloadUrl,setPayloadUrl] = useRecoilState(payloadUrlState)
const [payloadMethod,setPayloadMethod] = useRecoilState(payloadMethodState)

const [successStatus,setSuccessStatus] = useRecoilState(successValueState)
const [displayConfirmSaveDialog,setDisplayConfirmSaveDialog] = useRecoilState(displayConfirmSaveDialogState)

const saveData  = async ()=>{
  console.log(props.payloadData,'props.payloadData')
  let payload;

  const save = await apiRequest(props.method,props.endpointPath,props.payloadData)
  if(save){
