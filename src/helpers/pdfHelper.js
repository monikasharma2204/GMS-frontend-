...
export const downloadMemoInPdf = async (id, invoiceNo = "memo-in") => {
  if (!id) {
    alert("Invalid Memo In ID.");
    return;
  }

  try {
    const response = await axios.get(`${API_URL}/memo-in/${id}/pdf`, {
      responseType: "blob",
    });
    ...
    link.target = "_self";
    link.style.display = "none";
    link.setAttribute("download", `memo-in-${invoiceNo}.pdf`);
...
export const downloadMemoReturnPdf = async (id, invoiceNo = "memo-return") => {
  if (!id) {
    alert("Invalid Memo Return ID.");
    return;
  }

  try {
    const response = await axios.get(`${API_URL}/memo-return/${id}/pdf`, {
      responseType: "blob",
    });
    ...
    link.target = "_self";
    link.style.display = "none";
    link.download = `memo-return-${invoiceNo}.pdf`;
    ...
    console.error("Error downloading Memo Return PDF:", error);
    alert("Error downloading PDF. Please try again.");
  }
};
