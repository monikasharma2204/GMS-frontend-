...
import { downloadQuotationPdf } from "../../helpers/pdfHelper";
...
const ModalDayBook = ({
  ...
  const handleOKClick = () => {
    if (selectedItemIds.length === 1) {
      ...
      if (selectedItem) {
        ...
        const isSameInvoice = currentInvoiceId && selectedInvoiceId === currentInvoiceId;
        if (isSameInvoice) {
  ...
  const buildExportRows = () => {
    ...
    return rowsToExport.map((item) => {
      ...
      return {
        ...
        Account: (typeof item?.account === 'object' ? item?.account?.vendor_code_name : item?.account) || "",
        "Ref 1": item?.ref_1 || "",
        "Ref 2": item?.ref_2 || "",
        Pcs: summary?.pcs ?? 0,
  ...
  const handlePrint = async (e) => {
    ...
    const invoiceNo = item?.invoice_no || "quotation";
              ...
              >
                ...
                <Box sx={{ display: "flex", gap: "12px" }}>
                  ...
                  <Box
                    ...
                    sx={{
                      "&:hover svg path": {
                        fill: selectedItemIds.length === 1 ? "#E9B238" : "#666666",
                      },
                      marginTop: "6px",
                      "& svg path": {
                        fill: "#666666",
                      },
                      cursor: selectedItemIds.length === 1 ? "pointer" : "not-allowed",
                      opacity: selectedItemIds.length === 1 ? 1 : 0.5,
                    }}
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                  ...
                  data.map((item, index) => (
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          {typeof item.account === 'object' ? item.account?.vendor_code_name : item.account}
                        </Typography>
                      </Box>
