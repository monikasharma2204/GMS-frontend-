...
const QuotationsOrderBody = ({
  ...
  useEffect(() => {
    ...
    if (companyCurrency && !memoInfo?.currency && !memoInfo?.isDayBookEdit) {
      const setInitialCurrency = async () => {
        const currencyCode = await getCurrencyCodeById(apiRequest, companyCurrency);
        setMemoInfo((prev) => ({
          ...prev,
  ...
  }, [companyCurrency, memoInfo?.currency, memoInfo?.isDayBookEdit]);
