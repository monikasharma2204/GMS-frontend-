        ...
        <Box sx={{ display: "flex", alignItems: "center", gap: "12px" }}>
          <IconButton onClick={() => setShowSearch(!showSearch)} sx={{ p: 0 }}>
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              ...
              <path d="M20.9963 21.0002L16.6562 16.6602" stroke="#666666" strokeWidth="1.25" strokeLinecap="round" strokeLinejoin="round" />
      ...
      <Box sx={{ padding: "16px 16px 0px 16px", }}>
        ...
        <Box sx={{ borderRadius: "5px", marginTop: "8px" }}>
          ...
          <Box sx={{ display: "flex", height: "32px", }}>
            ...
            <Box sx={{ width: "56px", paddingRight: "14px", paddingLeft: "14px", borderBottom: "1px solid #EDEDED", textAlign: "right" }}>
              <Typography sx={{ fontSize: "14px", fontWeight: 700, fontFamily: "Calibri", color: "#343434" }}>Status</Typography>
            </Box>
          </Box>
          ...
          <Box sx={{ maxHeight: "calc(100vh - 200px)", overflow: "auto", mt: "8px" }}>
            {loading ? (
              ...
              filteredData.map((item) => {
                const transferItems = item.transfer_items || [];
                const totalPcs = transferItems.reduce((sum, s) => sum + (Number(s.pcs) || 0), 0);
                const totalWeight = transferItems.reduce((sum, s) => sum + (Number(s.weight) || 0), 0);
                const totalAmount = transferItems.reduce((sum, s) => sum + (Number(s.amount) || 0), 0);
                ...
                return (
                  ...
                  >
                    ...
                    <Typography sx={{ height: "38px", width: "80px", paddingLeft: "12px", paddingRight: "12px", fontSize: "14px", fontFamily: "Calibri", color: "#666", textAlign: "right" }}>{totalPcs}</Typography>
                    <Typography sx={{ height: "38px", width: "80px", paddingLeft: "12px", paddingRight: "12px", fontSize: "14px", fontFamily: "Calibri", color: "#666", textAlign: "right" }}>{totalWeight.toFixed(4)}</Typography>
                    <Typography sx={{ height: "38px", width: "120px", paddingLeft: "12px", paddingRight: "12px", fontSize: "14px", fontFamily: "Calibri", color: "#666", textAlign: "right" }}>{formatNumberWithCommas(totalAmount.toFixed(2))}</Typography>
