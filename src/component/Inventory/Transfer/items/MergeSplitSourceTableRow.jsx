...
const MergeSplitSourceTableRow = React.memo(({ item, index, onRemove, disabled = false }) => {
  const rowStyle = {
    height: "32px",
    display: "flex",
    alignItems: "center",
  ...
  const renderCell = (label, content, customStyle = {}) => (
    <Box sx={{
      ...
      borderRight: (label === "Lot" || label === "Weight") ? "1px solid #C6C6C8" : "1px solid #EDEDED",
      ...customStyle
    }}>
      {content}
    </Box>
  ...
  return (
    <Box sx={rowStyle}>
      {/* Remove Icon */}
      <Box sx={{ ...cellStyle, width: getWidth(""), borderRight: "1px solid #EDEDED" }}>
        <Box
      ...
      {renderCell("Color", item.color)}
      {renderCell("Cutting", item.cutting)}
      {renderCell("Quality", item.quality)}
      {renderCell("Clarity", item.clarity)}
