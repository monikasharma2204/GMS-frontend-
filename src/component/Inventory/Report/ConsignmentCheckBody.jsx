...
import ConsignmentCheckBodyInside from "./ConsignmentCheckBodyInside.jsx";
import { exportConsignmentToExcel } from "../../../helpers/consignmentExcelHelper";

const ConsignmentCheckBody = ({ exportTrigger }) => {
    ...
    const fetchStockMovements = async () => {
        try {
            ...
            console.error('Error fetching consignment movements:', err);
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleExportExcel = () => {
        try {
            ...
            const itemHeaders = [
                ["Memo No.", "Lot", "Stone", "Shape", "Size", "Color", "Cutting", "Quality", "Clarity", "In", "In", "Out", "Out", "Balance", "Balance"],
                ["", "", "", "", "", "", "", "", "", "Cts", "Pcs", "Cts", "Pcs", "Cts", "Pcs"]
            ];

            const itemRows = filteredStockMovements.map(item => [
                item.invoice_no || "",
                item.lot_no || "",
                item.stone || "",
                item.shape || "",
            ...
            exportConsignmentToExcel({
                filename: "Consignment Check List",
                sheetName: "Consignment Check",
                summaryHeaders: ["Total Items"],
                summaryValues: [stockMovements.length],
                itemHeaders,
                itemRows,
                merges: [
                    { s: { r: 2, c: 0 }, e: { r: 3, c: 0 } }, // Memo No.
                    { s: { r: 2, c: 1 }, e: { r: 3, c: 1 } }, // Lot
                    { s: { r: 2, c: 2 }, e: { r: 3, c: 2 } }, // Stone
                    { s: { r: 2, c: 3 }, e: { r: 3, c: 3 } }, // Shape
                    { s: { r: 2, c: 4 }, e: { r: 3, c: 4 } }, // Size
                    { s: { r: 2, c: 5 }, e: { r: 3, c: 5 } }, // Color
                    { s: { r: 2, c: 6 }, e: { r: 3, c: 6 } }, // Cutting
                    { s: { r: 2, c: 7 }, e: { r: 3, c: 7 } }, // Quality
                    { s: { r: 2, c: 8 }, e: { r: 3, c: 8 } }, // Clarity
                    { s: { r: 2, c: 9 }, e: { r: 2, c: 10 } }, // In
                    { s: { r: 2, c: 11 }, e: { r: 2, c: 12 } }, // Out
                    { s: { r: 2, c: 13 }, e: { r: 2, c: 14 } }, // Balance
                ]
            });
    ...
    useEffect(() => {
        if (exportTrigger > lastExportTrigger.current && !selectedItem) {
            handleExportExcel();
        }
        lastExportTrigger.current = exportTrigger;
    }, [exportTrigger, selectedItem]);
