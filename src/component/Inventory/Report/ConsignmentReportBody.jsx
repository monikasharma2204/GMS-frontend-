...
const ConsignmentReportBody = ({ exportTrigger }) => {
  ...
  const handleExportExcel = () => {
    try {
      if (filteredConsignmentList.length === 0) {
        alert("No data to export");
        return;
      }

      const itemHeaders = [
        "#", "Location", "Type", "Memo In NO.", "Doc Date", "Account", "Ref.", "Stone Code", "Stock ID",
        "Lot", "Stone", "Shape", "Size", "Color", "Cutting", "Quality", "Clarity", "Cer Type", "CerNo.",
        "Pcs", "Weight", "Price", "Unit", "Amount", "Currency", "Remark"
      ];

      const itemRows = filteredConsignmentList.map((row, rowIndex) => [
        rowIndex + 1,
        row.location || "N/A",
        row.type || "N/A",
        row.memo_no || "N/A",
