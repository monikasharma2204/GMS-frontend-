...
const getStatusFilterLabel = (item) => (item?.status_approve === "approved" ? "Approved" : "Unapproved");
...
const ModalDayBook = ({
  ...
  const { filteredData: accountFilteredData, handleAccountClick, popoverProps, isFilterActive } = useAccountFilter(data, selectedItems, open);
  const {
    filteredData,
  ...
  const handleOkClick = () => {
    if (selectedItems.length === 1) {
      ...
      const selectedInvoiceId = selectedItem._id || selectedItem.id;
      const isSameInvoice = currentInvoiceId && selectedInvoiceId && currentInvoiceId === selectedInvoiceId;

      if (isSameInvoice) {
        proceedWithSelection(selectedItem);
      } else if (isEditMode && hasUnsavedData && hasUnsavedData()) {
  ...
  const isAllSelected = sortedData && sortedData.length > 0 && selectedItemIds.length === sortedData.length;
...
const toFixedDecimal = (value, decimals) => {
  ...
  const buildExportRows = () => {
   ...
   Amount: toFixedDecimal(
  item?.summary?.[0]?.grand_total,
  2
),
  ...
  toFixedDecimal(item?.summary?.[0]?.grand_total, 2)
)}
                        </Typography>
                      </Box>
