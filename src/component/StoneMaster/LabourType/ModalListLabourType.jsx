...
const ModalListLabourType = () => {
    ...
    const fetchData = async () => {
      try {
        const response = await apiRequest(
          "GET",
          "/master?master_type=master_labour_type"
        );
        setData(response);
      } catch (err) {
        setError(err);
  ...
  const handleSwitchChange = (id) => async (event) => {
    ...
    try {
      ...
      await apiRequest("PUT", "/master?master_type=master_labour_type", {
        _id: id,
...
export default ModalListLabourType;
