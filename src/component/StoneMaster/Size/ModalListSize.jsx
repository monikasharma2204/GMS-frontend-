...
import IOSSwitchInModal from "../../SwitchIOSStyleInModal";
...
const ModalListSize = () => {
    ...
    const fetchData = async () => {
      try {
        const response = await apiRequest(
          "GET",
          "/master?master_type=master_stone_size"
        );
        setData(response);
      } catch (err) {
        setError(err);
  ...
  useEffect(() => {
    const fetchShapeNames = async () => {
      try {
        const response = await axios.get(
          API_URL+"/master?master_type=master_stone_shape"
        );
        const shapes = response.data;
        const shapeNameMap = shapes.reduce((acc, shape) => {
          acc[shape._id] = shape.code;
          return acc;
        }, {});
        setShapeNames(shapeNameMap);
      } catch (error) {
        console.error("Error fetching shape code:", error);
      }
    };
  ...
  const handleSwitchChange = (id) => async (event) => {
    ...
    setData((prevData) => {
      const updatedData = prevData.map((item) =>
        item._id === id ? { ...item, master_status: newStatus } : item
      );
    ...
    try {
      ...
      await apiRequest("PUT", "/master?master_type=master_stone_group", {
        _id: id,
                  ...
                  {data.map((item, index) => (
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          {item.master_info && item.master_info.carat_size
                            ? item.master_info.carat_size
                            : "N/A"}
                        </Typography>
                      </Box>
                      ...
                      >
                        ...
                        >
                          {item.master_info?.master_shapes?.length > 0
                            ? item.master_info.master_shapes
                                .map(
                                  (shapeId) => shapeNames[shapeId] || shapeId
                                )
                                .join(", ")
                            : "No shapes available"}
                        </Typography>
