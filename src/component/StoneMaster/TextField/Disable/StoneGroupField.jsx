...
const StoneGroupField = ({
  ...
  const handleStoneGroupChange = (event) => {
    ...
    const selectedOption = options.find(
      (option) => option.value === newCodeValue
    );
  ...
  useEffect(() => {
    ...
    const shouldReset = (defaultValue === null || defaultValue === undefined) && 
                        (!selectedData || !selectedData.stone_group_id);
    ...
    axios
      ...
      .then((response) => {
        if (isMounted) {
          ...
          setOptions(fetchedOptions.filter((option) => option.status === "active"));  
          const defaultItem = fetchedOptions.find(
            (option) => option._id === selectedData?.stone_group_code
          );
          console.log(selectedData);
          if (selectedData?.stone_group_code && !selectedValue) {
            const defaultItem = fetchedOptions.find(
              (option) => option._id === selectedData.stone_group_id
            );
            if (defaultItem) {
  ...
  return (
    ...
    >
      ...
      <FormControl
        sx={{ width: "328px", backgroundColor: isEditing ? "#FFF" : "#F0F0F0" }}
      >
        <Select
          ref={selectRef}
          disabled={!isEditing}
