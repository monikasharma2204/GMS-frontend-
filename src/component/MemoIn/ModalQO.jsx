...
import { openModalPurchaseOrderState,purchaseOrderListState ,memoItemState,purchaseOrderSelectedListState,purchaseOrderIdListSelectedItemState,currentPurchaseOrderListSelectedItemState,selectedCurrencyState, useDiscountPercentState,useDiscountAmountState,useVATState,vatAmountState,otherChargeState,discountPercentState,memoInfoState,discountAmountState ,discountAmountTotalState,subTotalState,keyEditState,vatPercentState,grandTotalState,editMemoStatusState,openModalMemoPendingOrderState,memoPendingListState ,memoPendingListSelectedItemState,memoPendingIdListSelectedItemState,currentPendingListSelectedItemState,currentDiscountValueState} from "recoil/MemoIn/MemoState.js";
...
const ModalMemoPen = (props) => {
  ...
  const [openModalPurchaseOrder,setOpenModalPurchaseOrder] = useRecoilState(openModalPurchaseOrderState)

  
  const [purchaseOrderList,setPurchaseOrderList] = useRecoilState(purchaseOrderListState)

  const [purchaseOrderSelectedList,setPurchaseOrderSelectedList] = useRecoilState(purchaseOrderSelectedListState)

  const [purchaseOrderIdListSelectedItem,setPurchaseOrderIdListSelectedItem] = useRecoilState(purchaseOrderIdListSelectedItemState)

  const [currentPurchaseOrderListSelectedItem,setCurrentPurchaseOrderListSelectedItem] = useRecoilState(currentPurchaseOrderListSelectedItemState)
...
const [currentDiscountValue,setCurrentDiscountValue] = useRecoilState(currentDiscountValueState)
  ...
  const getInventoryData = async () => {
    ...
    try {
      const {data} = await apiRequest("GET", "/inventory-in/list/po-list", {}, { params: { inventory_type: "purchase_po" } })
      let item = []
  ...
  useEffect(() => {
    ...
    }, [inventoryData,purchaseOrderIdListSelectedItem]);

  const handleCheckboxChange = (e,key,itemData) => {
if(e.target.checked){

    setPurchaseOrderIdListSelectedItem((prev)=>{
    ...
    setCurrentPurchaseOrderListSelectedItem((prev)=>{
  ...
  const handleModalSave = () => {
    setOpenModalPurchaseOrder(false)
    let dataUpdate;
    let prevDataList ; 
    ...
    setMemoItem((prev)=>{
      ...
      if(memoItem[0].stone===""){
        dataUpdate = [...currentPurchaseOrderListSelectedItem]

      return dataUpdate

      }else{
        dataUpdate = [...prev,...currentPurchaseOrderListSelectedItem]
        return dataUpdate
  ...
  return (
    <>
      <Button
  onClick={()=>{
    setOpenModalPurchaseOrder(true)
  }}
  sx={{
    textTransform: "none",
    height: "35px",
    ...
    >
      ...
      >
        ...
        >
          ...
          >
         <Checkbox
                   checked={purchaseOrderIdListSelectedItem.some(
