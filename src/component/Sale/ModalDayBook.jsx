...
const ModalDayBook = ({
  ...
  const { filteredData, handleAccountClick, popoverProps, isFilterActive } = useAccountFilter(data, selectedItems, open);
  ...
  const isAllSelected = hasData && sortedData.every((item) => item.checked);
  ...
  const buildExportRows = () => {
    const rowsToExport =
      selectedItems.length > 0 ? selectedItems : sortedData || [];
    ...
    return rowsToExport.map((item, exportIndex) => {
      ...
      return {
        ...
        Account: item?.vendor_code_name || (typeof item?.account === 'object' ? item?.account?.vendor_code_name : item?.account) || "",
        "Ref 1": item?.ref_1 || "",
        "Ref 2": item?.ref_2 || "",
        Pcs: summary?.pcs ?? 0,
                  ...
                  sortedData.map((item, index) => (
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          {item.vendor_code_name || (typeof item.account === 'object' ? item.account?.vendor_code_name : item.account)}
                        </Typography>
                      </Box>
