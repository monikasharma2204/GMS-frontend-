...
import { payableFormDataState, payableValidationErrorState, payableFSMState } from "../../../recoil/state/FinanceState";
...
const PayableFooter = () => {
  ...
  const setValidationError = useSetRecoilState(payableValidationErrorState);
  ...
  const handleSaveClick = () => {
    ...
    const grandTotal = formData.items.reduce((sum, item) => sum + (parseFloat(item.paid) || 0), 0);
    const paymentTotal =
  ...
  const handleSave = async (confirmed) => {
    ...
    try {
      ...
      if (slipImage instanceof File) {
        ...
        payload.append("invoice_no", formData.payableNo || "");
        payload.append("vendor_code_id", formData.account?._id || formData.account?.id || "");
        ...
        const grandTotalVal = formData.items.reduce((sum, item) => sum + (parseFloat(item.paid) || 0), 0).toFixed(2);
        ...
        payload = {
          invoice_no: formData.payableNo || "",
          vendor_code_id: formData.account?._id || formData.account?.id || "",
          ...
          payment_method: {
            ...
            grand_total: formData.items.reduce((sum, item) => sum + (parseFloat(item.paid) || 0), 0).toFixed(2)
          },
          ref_1: formData.ref1 || "",
          ref_2: formData.ref2 || "",
          note: formData.note || ""
      ...
      const response = await apiRequest("POST", "/payables", payload);
      if (response) {
        setSuccessOpen(true);
        setFsmState("view");
      ...
      alert("Error saving payable: " + (error.response?.data?.message || error.message));
    }
  };
  ...
  return (
    <>
      ...
      <CustomConfirmDialog
        ...
        title="Confirm Payable Cancellation"
        subtitle={
