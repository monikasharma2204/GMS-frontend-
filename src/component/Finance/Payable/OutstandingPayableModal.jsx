import React, { useState, useEffect } from "react";
import {
  Modal,
  Box,
  Typography,
  Checkbox,
  FormControl,
  Select,
  MenuItem,
  TextField,
  InputAdornment,
  Button,
} from "@mui/material";
import SearchIcon from "@mui/icons-material/Search";
import moment from "moment";
import { useRecoilState, useRecoilValue } from "recoil";
import { payableOutstandingModalOpenState, payableFormDataState } from "../../../recoil/state/FinanceState";
import apiRequest from "../../../helpers/apiHelper";
import { formatNumberWithCommas } from "../../../helpers/numberHelper.js";

const style = {
  position: "absolute",
  top: "50%",
  left: "50%",
  transform: "translate(-50%, -50%)",
  width: 1360,
  height: 842,
  bgcolor: "background.paper",
  borderRadius: "8px",
  outline: "none",
  display: "flex",
  flexDirection: "column",
};

const OutstandingPayableModal = () => {
  const [open, setOpen] = useRecoilState(payableOutstandingModalOpenState);
  const [formData, setFormData] = useRecoilState(payableFormDataState);
  const [data, setData] = useState([]);
  const [loading, setLoading] = useState(false);
  const [rowsPerPage, setRowsPerPage] = useState(10);
  const [selectedItemIds, setSelectedItemIds] = useState([]);
  const [searchQuery, setSearchQuery] = useState("");

  useEffect(() => {
    if (open) {
      fetchOutstanding();
    }
  }, [open]);

  const fetchOutstanding = async () => {
    if (!formData.account) return;
    setLoading(true);
    try {
      const accountName = formData.account.vendor_code_name || formData.account.label;
      const response = await apiRequest("GET", `/pu/outstandingpayable?account=${accountName}`);
      if (Array.isArray(response)) {
        setData(response);
      } else if (response?.data) {
        setData(response.data);
      }
    } catch (error) {
      console.error("Failed to fetch outstanding payables:", error);
    } finally {
      setLoading(false);
    }
  };

  const handleClose = () => {
    setOpen(false);
    setSelectedItemIds([]);
  };

  const handleSelectAll = () => {
    if (selectedItemIds.length > 0) {
      setSelectedItemIds([]);
    } else if (data.length > 0) {
      let restrictedCurrency = null;
      if (formData.items.length > 0) {
        restrictedCurrency = formData.currency;
      } else if (selectedItemIds.length > 0) {
        const firstSelectedItem = data.find(item => selectedItemIds.includes(item._id || item.id));
        restrictedCurrency = firstSelectedItem?.currency?.code || firstSelectedItem?.currency;
      } else {
        restrictedCurrency = data[0].currency?.code || data[0].currency;
      }

      const matchingItems = data.filter(item => {
        const itemCurrency = item.currency?.code || item.currency;
        const alreadyAdded = formData.items.some(fItem => fItem.purchaseNo === (item.invoice_no || item.purchaseNo));
        return itemCurrency === restrictedCurrency && !alreadyAdded;
      });
      setSelectedItemIds(matchingItems.map((item) => item._id || item.id));
    }
  };

  const handleCheckboxChange = (row) => {
    const id = row._id || row.id;
    const currency = row.currency?.code || row.currency;

    const isAlreadyAdded = formData.items.some(item => item.purchaseNo === (row.invoice_no || row.purchaseNo));
    if (isAlreadyAdded) return;

    if (selectedItemIds.length > 0) {
      const firstSelectedItem = data.find(item => selectedItemIds.includes(item._id || item.id));
      const firstCurrency = firstSelectedItem?.currency?.code || firstSelectedItem?.currency;
      if (currency !== firstCurrency) {
        return;
      }
    }

    setSelectedItemIds((prev) =>
      prev.includes(id) ? prev.filter((item) => item !== id) : [...prev, id]
    );
  };

  const handleChange = (e) => setRowsPerPage(e.target.value);

  const isAllSelected = data.length > 0 && selectedItemIds.length === data.length;

  return (
    <Modal open={open} onClose={handleClose}>
      <Box sx={style}>
        {/* Header */}
        <Box
          sx={{
            width: "100%",
            height: "56px",
            backgroundColor: "#05595B",
            borderTopLeftRadius: "8px",
            borderTopRightRadius: "8px",
            justifyContent: "space-between",
            display: "flex",
            alignItems: "center",
            padding: "0 24px",
            boxSizing: "border-box",
          }}
        >
          <Typography sx={{ color: "#FFF", fontFamily: "Calibri", fontSize: "24px", fontWeight: 900 }}>
            Outstanding Payable
          </Typography>
          <Box onClick={handleClose} sx={{ cursor: "pointer", display: "flex", alignItems: "center" }}>
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M14.1535 12.0008L19.5352 6.61748C19.6806 6.47704 19.7966 6.30905 19.8764 6.12331C19.9562 5.93757 19.9982 5.7378 19.9999 5.53565C20.0017 5.3335 19.9632 5.13303 19.8866 4.94593C19.8101 4.75883 19.697 4.58885 19.5541 4.44591C19.4111 4.30296 19.2412 4.18992 19.0541 4.11337C18.867 4.03682 18.6665 3.9983 18.4644 4.00006C18.2622 4.00181 18.0624 4.04381 17.8767 4.1236C17.691 4.20339 17.523 4.31937 17.3825 4.46478L11.9992 9.84654L6.61748 4.46478C6.47704 4.31937 6.30905 4.20339 6.12331 4.1236C5.93757 4.04381 5.7378 4.00181 5.53565 4.00006C5.3335 3.9983 5.13303 4.03682 4.94593 4.11337C4.75883 4.18992 4.58885 4.30296 4.44591 4.44591C4.30296 4.58885 4.18992 4.75883 4.11337 4.94593C4.03682 5.13303 3.9983 5.3335 4.00006 5.53565C4.00181 5.7378 4.04381 5.93757 4.1236 6.12331C4.20339 6.30905 4.31937 6.47704 4.46478 6.61748L9.84654 11.9992L4.46478 17.3825C4.31937 17.523 4.20339 17.691 4.1236 17.8767C4.04381 18.0624 4.00181 18.2622 4.00006 18.4644C3.9983 18.6665 4.03682 18.867 4.11337 19.0541C4.18992 19.2412 4.30296 19.4111 4.44591 19.5541C4.58885 19.697 4.75883 19.8101 4.94593 19.8866C5.13303 19.9632 5.3335 20.0017 5.53565 19.9999C5.7378 19.9982 5.93757 19.9562 6.12331 19.8764C6.30905 19.7966 6.47704 19.6806 6.61748 19.5352L11.9992 14.1535L17.3825 19.5352C17.523 19.6806 17.691 19.7966 17.8767 19.8764C18.0624 19.9562 18.2622 19.9982 18.4644 19.9999C18.6665 20.0017 18.867 19.9632 19.0541 19.8866C19.2412 19.8101 19.4111 19.697 19.5541 19.5541C19.697 19.4111 19.8101 19.2412 19.8866 19.0541C19.9632 18.867 20.0017 18.6665 19.9999 18.4644C19.9982 18.2622 19.9562 18.0624 19.8764 17.8767C19.7966 17.691 19.6806 17.523 19.5352 17.3825L14.1535 12.0008Z" fill="white" />
            </svg>
          </Box>
        </Box>

        {/* Body Container */}
        <Box
          sx={{
            backgroundColor: "#F8F8F8",
            flexGrow: 1,
            margin: "33px",
            padding: "32px",
            boxSizing: "border-box",
            borderRadius: "8px",
            display: "flex",
            flexDirection: "column",
          }}
        >
          {/* Toolbar */}
          <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 3 }}>
            <Typography sx={{ color: "#343434", fontFamily: "Calibri", fontSize: "20px", fontWeight: 700 }}>
              Outstanding Payable List
            </Typography>
            <Box sx={{ display: "flex", gap: "12px" }}>
              <Box
                sx={{
                  width: "113px",
                  height: "38px",
                  display: "flex",
                  flexDirection: "column",
                  justifyContent: "center",
                }}
              >
                <Typography
                  sx={{
                    color: "#343434",
                    fontFamily: "Calibri",
                    fontSize: "16px",
                    fontWeight: 400,
                  }}
                >
                  Rows per page
                </Typography>
              </Box>

              <FormControl sx={{ height: "40px", width: "69px", marginRight: "10px" }}>
                <Select
                  sx={{
                    height: "40px",
                    width: "69px",
                    backgroundColor: "#FFF",
                    color: "#343434",
                    fontFamily: "Calibri",
                    fontSize: "16px",
                    fontWeight: 400,
                  }}
                  value={rowsPerPage}
                  onChange={handleChange}
                >
                  <MenuItem value={10}>10</MenuItem>
                  <MenuItem value={20}>20</MenuItem>
                  <MenuItem value={30}>30</MenuItem>
                </Select>
              </FormControl>

              <Box sx={{ "&:hover svg path": { fill: "#E9B238" }, marginTop: "6px", "& svg path": { fill: "#666666" } }}>
                <svg width="28" height="28" viewBox="0 0 28 28" fill="none">
                  <path d="M8.16671 2.33398C7.85729 2.33398 7.56054 2.4569 7.34175 2.67569C7.12296 2.89449 7.00004 3.19123 7.00004 3.50065V8.16732H4.66671C4.04787 8.16732 3.45438 8.41315 3.01679 8.85074C2.57921 9.28832 2.33337 9.88181 2.33337 10.5007V19.834C2.33337 20.4528 2.57921 21.0463 3.01679 21.4839C3.45438 21.9215 4.04787 22.1673 4.66671 22.1673H7.00004V24.5007C7.00004 24.8101 7.12296 25.1068 7.34175 25.3256C7.56054 25.5444 7.85729 25.6673 8.16671 25.6673H19.8334C20.1428 25.6673 20.4395 25.5444 20.6583 25.3256C20.8771 25.1068 21 24.8101 21 24.5007V22.1673H23.3334C23.9522 22.1673 24.5457 21.9215 24.9833 21.4839C25.4209 21.0463 25.6667 20.4528 25.6667 19.834V10.5007C25.6667 9.88181 25.4209 9.28832 24.9833 8.85074C24.5457 8.41315 23.9522 8.16732 23.3334 8.16732H21V3.50065C21 3.19123 20.8771 2.89449 20.6583 2.67569C20.4395 2.4569 20.1428 2.33398 19.8334 2.33398H8.16671ZM19.8334 16.334H8.16671C7.85729 16.334 7.56054 16.4569 7.34175 16.6757C7.12296 16.8945 7.00004 17.1912 7.00004 17.5007V19.834H4.66671V10.5007H23.3334V19.834H21V17.5007C21 17.1912 20.8771 16.8945 20.6583 16.6757C20.4395 16.4569 20.1428 16.334 19.8334 16.334ZM18.6667 8.16732H9.33337V4.66732H18.6667V8.16732ZM5.83337 11.6673V14.0007H9.33337V11.6673H5.83337ZM18.6667 18.6673V23.334H9.33337V18.6673H18.6667Z" fill="white" />
                </svg>
              </Box>

              <Box sx={{ "&:hover svg path": { fill: "#00AA3A" }, marginTop: "6px", "& svg path": { fill: "#666666" } }}>
                <svg width="25" height="25" viewBox="0 0 25 25" fill="none">
                  <path d="M19.4175 1.7L17.8862 0H5.8275C4.9575 0 4.62125 0.645 4.62125 1.14875V5.68625H6.3125V2.06625C6.3125 1.87375 6.475 1.71125 6.6625 1.71125H15.2913C15.4812 1.71125 15.5763 1.745 15.5763 1.90125V7.92625H21.7175C21.9587 7.92625 22.0525 8.05125 22.0525 8.23375V22.9463C22.0525 23.2537 21.9275 23.3 21.74 23.3H6.6625C6.56952 23.2977 6.48106 23.2595 6.41576 23.1932C6.35047 23.127 6.31345 23.038 6.3125 22.945V21.6H4.6325V23.7188C4.61 24.4688 5.01 25 5.8275 25H22.575C23.45 25 23.7488 24.3663 23.7488 23.7887V6.48375L23.3113 6.00875L19.4175 1.7ZM17.295 1.9L17.7787 2.4425L21.0238 6.00875L21.2025 6.225H17.8862C17.6362 6.225 17.4775 6.18375 17.4112 6.1C17.345 6.01875 17.3062 5.8875 17.295 5.70875V1.9ZM15.9325 13.3337H21.6537V15.0013H15.9312L15.9325 13.3337ZM15.9325 10.0013H21.6537V11.6675H15.9312L15.9325 10.0013ZM15.9325 16.6675H21.6537V18.335H15.9312L15.9325 16.6675ZM1.25 7.0325V20.3662H14.3313V7.0325H1.25ZM7.79125 14.7875L6.99125 16.01H7.79125V17.5H3.77L6.6875 13.1125L4.1025 9.1675H6.2625L7.7925 11.4625L9.32125 9.1675H11.48L8.89 13.1125L11.8113 17.5H9.57L7.79125 14.7875Z" fill="white" />
                </svg>
              </Box>
            </Box>
          </Box>

          {/* Table Container */}
          <Box
            sx={{
              width: "100%",
              flexGrow: 1,
              bgcolor: "#FFF",
              borderRadius: "5px",
              border: "1px solid #C6C6C8",
              overflow: "auto",
              position: "relative",
            }}
          >
            {/* Table Header */}
            <Box sx={{ display: "flex", bgcolor: "#EDEDED", height: "38px", position: "sticky", top: 0, zIndex: 1, borderBottom: "1px solid #C6C6C8" }}>
              <Box sx={{ width: "60px", display: "flex", alignItems: "center" }}>
                <Checkbox size="small" checked={isAllSelected} onChange={handleSelectAll} />
                <Typography sx={{ fontFamily: "Calibri", fontWeight: 700, fontSize: "16px", color: "#343434" }}>#</Typography>
              </Box>
              <Box sx={{ width: "200px", display: "flex", alignItems: "center", pl: 2, }}>
                <Typography sx={{ fontFamily: "Calibri", fontWeight: 700, fontSize: "16px", color: "#343434" }}>Purchase No.</Typography>
              </Box>
              <Box sx={{ width: "250px", display: "flex", alignItems: "center", pl: 2 }}>
                <Typography sx={{ fontFamily: "Calibri", fontWeight: 700, fontSize: "16px", color: "#343434" }}>Vendor Name</Typography>
              </Box>
              <Box sx={{ width: "140px", display: "flex", alignItems: "center", justifyContent: "center" }}>
                <Typography sx={{ fontFamily: "Calibri", fontWeight: 700, fontSize: "16px", color: "#343434" }}>Doc Date</Typography>
              </Box>
              <Box sx={{ width: "140px", display: "flex", alignItems: "center", justifyContent: "right", pr: 2 }}>
                <Typography sx={{ fontFamily: "Calibri", fontWeight: 700, fontSize: "16px", color: "#343434" }}>Balance</Typography>
              </Box>
              <Box sx={{ width: "100px", display: "flex", alignItems: "center", justifyContent: "center" }}>
                <Typography sx={{ fontFamily: "Calibri", fontWeight: 700, fontSize: "16px", color: "#343434" }}>Currency</Typography>
              </Box>
              <Box sx={{ flex: 1, display: "flex", alignItems: "center", pl: 2 }}>
                <Typography sx={{ fontFamily: "Calibri", fontWeight: 700, fontSize: "16px", color: "#343434" }}>Remark</Typography>
              </Box>
            </Box>

            {/* Table Body */}
            <Box>
              {data.length > 0 ? (
                data.map((row, index) => {
                  const isSelected = selectedItemIds.includes(row._id || row.id);
                  const currency = row.currency?.code || row.currency;
                  const purchaseNo = row.invoice_no || row.purchaseNo;
                  const isAlreadyAdded = formData.items.some(item => item.purchaseNo === purchaseNo);
                  
                  let isDisableSelection = isAlreadyAdded;

                  if (!isDisableSelection && selectedItemIds.length > 0) {
                    const firstSelectedItem = data.find(item => selectedItemIds.includes(item._id || item.id));
                    const firstCurrency = firstSelectedItem?.currency?.code || firstSelectedItem?.currency;
                    if (currency !== firstCurrency) {
                      isDisableSelection = true;
                    }
                  }

                  return (
                    <Box key={index} sx={{
                      display: "flex",
                      height: "38px",
                      borderBottom: "1px solid #EDEDED",
                      "&:hover": { bgcolor: isDisableSelection ? "transparent" : "#F5F5F5" },
                      opacity: isDisableSelection ? 0.5 : 1,
                      backgroundColor: isAlreadyAdded ? "#F0F0F0" : "transparent"
                    }}>
                      <Box sx={{ width: "60px", display: "flex", alignItems: "center" }}>
                        <Checkbox
                          size="small"
                          checked={isSelected || isAlreadyAdded}
                          disabled={isDisableSelection && !isSelected}
                          onChange={() => handleCheckboxChange(row)}
                        />
                        <Typography sx={{ fontFamily: "Calibri", fontSize: "16px", fontWeight: 400, color: "#343434" }}>{index + 1}</Typography>
                      </Box>
                      <Box sx={{ width: "200px", display: "flex", alignItems: "center", pl: 2 }}>
                        <Typography sx={{ fontFamily: "Calibri", fontSize: "16px", fontWeight: 400, color: "#343434" }}>{purchaseNo}</Typography>
                      </Box>
                      <Box sx={{ width: "250px", display: "flex", alignItems: "center", pl: 2 }}>
                        <Typography sx={{ fontFamily: "Calibri", fontSize: "16px", fontWeight: 400, color: "#343434" }}>{row.account?.vendor_code_name || row.vendorName}</Typography>
                      </Box>
                      <Box sx={{ width: "140px", display: "flex", alignItems: "center", justifyContent: "center" }}>
                        <Typography sx={{ fontFamily: "Calibri", fontSize: "16px", fontWeight: 400, color: "#343434" }}>{moment(row.doc_date || row.docDate).format("DD/MM/YYYY")}</Typography>
                      </Box>
                      <Box sx={{ width: "140px", display: "flex", alignItems: "center", justifyContent: "right", pr: 2 }}>
                        <Typography sx={{ fontFamily: "Calibri", fontSize: "16px", fontWeight: 400, color: "#343434" }}>{formatNumberWithCommas(parseFloat(row.outstandingpayable || row.balance || 0).toFixed(2))}</Typography>
                      </Box>
                      <Box sx={{ width: "100px", display: "flex", alignItems: "center", justifyContent: "center" }}>
                        <Typography sx={{ fontFamily: "Calibri", fontSize: "16px", fontWeight: 400, color: "#343434" }}>{currency}</Typography>
                      </Box>
                      <Box sx={{ flex: 1, display: "flex", alignItems: "center", pl: 2 }}>
                        <Typography sx={{ fontFamily: "Calibri", fontSize: "16px", fontWeight: 400, color: "#343434" }}>{row.remark}</Typography>
                      </Box>
                    </Box>
                  );
                })
              ) : (
                <Box sx={{ display: "flex", justifyContent: "center", alignItems: "center", height: "100%", py: 10 }}>
                  <Typography sx={{ color: "#9A9A9A", fontFamily: "Calibri", fontSize: "18px" }}>No data found</Typography>
                </Box>
              )}
            </Box>
          </Box>

          {/* Footer Buttons */}
          <Box sx={{ display: "flex", justifyContent: "flex-end", gap: "12px", mt: 3 }}>
            <Button
              onClick={handleClose}
              sx={{
                width: "96px",
                height: "35px",
                borderRadius: "4px",
                border: "1px solid #BFBFBF",
                bgcolor: "#FFF",
                color: "#343434",
                textTransform: "none",
                fontFamily: "Calibri",
                fontWeight: 700,
                fontSize: "16px"
              }}
            >
              Cancel
            </Button>
            <Button
              disabled={selectedItemIds.length === 0}
              onClick={() => {
                const selectedRows = data.filter(item => selectedItemIds.includes(item._id || item.id));
                const newRows = selectedRows.filter(row =>
                  !formData.items.some(item => item.purchaseNo === (row.invoice_no || row.purchaseNo))
                );

                const mappedItems = newRows.map(row => ({
                  _id: row._id || row.id,
                  purchaseNo: row.invoice_no || row.purchaseNo,
                  outstanding: row.outstandingpayable || row.balance || 0,
                  paid: 0,
                  balance: row.outstandingpayable || row.balance || 0,
                  remark: ""
                }));

                if (mappedItems.length === 0) {
                  handleClose();
                  return;
                }

                const row = selectedRows[0];
                const currencyCode = row?.currency?.code || row?.code || (typeof row?.currency === 'string' ? row.currency : null);
                const currencyId = row?.currency?._id || row?.currency?.id || null;
                const isCurrencyChanged = formData.items.length > 0 && currencyCode !== formData.currency;

                setFormData(prev => ({
                  ...prev,
                  items: isCurrencyChanged ? mappedItems : [...prev.items, ...mappedItems],
                  currency: currencyCode,
                  currencyId: currencyId
                }));
                handleClose();
              }}
              sx={{
                width: "79px",
                height: "35px",
                borderRadius: "4px",
                bgcolor: "#17C653",
                color: "#FFF",
                textTransform: "none",
                fontFamily: "Calibri",
                fontWeight: 700,
                fontSize: "16px",
                "&:hover": { bgcolor: "#15B049" },
                "&.Mui-disabled": { bgcolor: "#A5D6B1", color: "#FFF" }
              }}
            >
              Ok
            </Button>
          </Box>
        </Box>
      </Box>
    </Modal>
  );
};

export default OutstandingPayableModal;
