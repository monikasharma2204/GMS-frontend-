...
import { outstandingModalOpenState, receivableFormDataState } from "../../../recoil/state/FinanceState";
...
const OutstandingReceivableModal = () => {
  const [open, setOpen] = useRecoilState(outstandingModalOpenState);
  ...
  const fetchOutstanding = async () => {
    ...
    try {
      ...
      const response = await apiRequest("GET", `/sales/outstandingreceivables?account=${accountName}`);
  ...
  const handleSelectAll = () => {
    if (selectedItemIds.length > 0) {
      ...
      const matchingItems = data.filter(item => {
        ...
        const alreadyAdded = formData.items.some(fItem => fItem.saleNo === (item.invoice_no || item.saleNo));
  ...
  const handleCheckboxChange = (row) => {
    ...
    const isAlreadyAdded = formData.items.some(item => item.saleNo === (row.invoice_no || row.saleNo));
          ...
          >
            ...
            <Box sx={{ display: "flex", bgcolor: "#EDEDED", height: "38px", position: "sticky", top: 0, zIndex: 1, borderBottom: "1px solid #C6C6C8" }}>
              ...
              <Box sx={{ width: "200px", display: "flex", alignItems: "center", pl: 2, }}>
                <Typography sx={{ fontFamily: "Calibri", fontWeight: 700, fontSize: "16px", color: "#343434" }}>Sale No.</Typography>
              </Box>
              <Box sx={{ width: "250px", display: "flex", alignItems: "center", pl: 2 }}>
                <Typography sx={{ fontFamily: "Calibri", fontWeight: 700, fontSize: "16px", color: "#343434" }}>Customer Name</Typography>
              </Box>
                ...
                data.map((row, index) => {
                  ...
                  return (
                    <Box key={index} sx={{
                      ...
                      <Box sx={{ width: "200px", display: "flex", alignItems: "center", pl: 2 }}>
                        <Typography sx={{ fontFamily: "Calibri", fontSize: "16px", fontWeight: 400, color: "#343434" }}>{row.invoice_no || row.saleNo}</Typography>
                      </Box>
                      <Box sx={{ width: "250px", display: "flex", alignItems: "center", pl: 2 }}>
                        <Typography sx={{ fontFamily: "Calibri", fontSize: "16px", fontWeight: 400, color: "#343434" }}>{row.account?.vendor_code_name || row.customerName}</Typography>
                      </Box>
                      <Box sx={{ width: "140px", display: "flex", alignItems: "center", justifyContent: "center" }}>
                      ...
                      <Box sx={{ width: "140px", display: "flex", alignItems: "center", justifyContent: "right", pr: 2 }}>
                        <Typography sx={{ fontFamily: "Calibri", fontSize: "16px", fontWeight: 400, color: "#343434" }}>{formatNumberWithCommas(parseFloat(row.outstandingreceivable || row.balance || 0).toFixed(2))}</Typography>
                      </Box>
                      <Box sx={{ width: "100px", display: "flex", alignItems: "center", justifyContent: "center" }}>
            ...
            <Button
              ...
              onClick={() => {
                ...
                const newRows = selectedRows.filter(row =>
                  !formData.items.some(item => item.saleNo === (row.invoice_no || row.saleNo))
                );

                const mappedItems = newRows.map(row => ({
                  _id: row._id || row.id,
                  saleNo: row.invoice_no || row.saleNo,
                  outstanding: row.outstandingreceivable || row.balance || 0,
                  receive: 0,
                  balance: row.outstandingreceivable || row.balance || 0,
                  remark: ""
                }));
...
export default OutstandingReceivableModal;
