...
import { receivableFormDataState, outstandingModalOpenState, receivableValidationErrorState, bankImageModalOpenState, receivableFSMState } from "../../../recoil/state/FinanceState";
...
const ReceivableBody = () => {
  ...
  const setOutstandingOpen = useSetRecoilState(outstandingModalOpenState);
  const [open, setOpen] = useRecoilState(bankImageModalOpenState);
  const [fsmState, setFsmState] = useRecoilState(receivableFSMState);
  const [validationError, setValidationError] = useRecoilState(receivableValidationErrorState);
  const [customerList, setCustomerList] = useState([]);
  const [bankThumbnailUrl, setBankThumbnailUrl] = useState(null);

  const resetFormData = useResetRecoilState(receivableFormDataState);
  const resetFsmState = useResetRecoilState(receivableFSMState);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Fetch Customers
        const customerResponse = await apiRequest("GET", "/account/customer/list");
        if (Array.isArray(customerResponse)) {
          const mappedCustomers = customerResponse
        ...
        const codeResponse = await apiRequest("GET", "/receivables/next-invoice-no");
        if (codeResponse?.data) {
          setFormData(prev => ({
            ...prev,
            receivableNo: codeResponse.data,
            docDate: prev.docDate || new Date()
          }));
        } else if (codeResponse?.invoice_no) {
          setFormData(prev => ({
            ...prev,
            receivableNo: codeResponse.invoice_no,
            docDate: prev.docDate || new Date()
          }));
        } else if (typeof codeResponse === "string") {
          setFormData(prev => ({
            ...prev,
            receivableNo: codeResponse,
            docDate: prev.docDate || new Date()
          }));
        }
      } catch (error) {
  ...
  const isOutstandingDisabled = !formData.account;

  const handleItemChange = (index, field, value) => {
    const rawValue = field === "receive" ? value.toString().replace(/,/g, "") : value;
    const newItems = [...formData.items];
    const item = { ...newItems[index], [field]: rawValue };

    if (field === "receive") {
      const receiveVal = parseFloat(rawValue) || 0;
      const outstandingVal = parseFloat(item.outstanding) || 0;

      if (receiveVal > outstandingVal) {
        setValidationError(`Receive amount exceeds outstanding balance for sale ${item.saleNo}`);
      } else {
        setValidationError(null);
      }

      item.balance = (outstandingVal - receiveVal).toFixed(2);
    }
  ...
  const grandTotal = calculateTotal("receive");
    ...
    >
      <Box sx={{ display: "flex", justifyContent: "flex-end" }}>
        <Typography sx={{ color: "#9A9A9A", fontSize: "12px", fontFamily: "Calibri" }}>
          Transaction Date : 06/11/2025 By : Super Admin
        </Typography>
      </Box>
      ...
      >
        ...
        <Box sx={{ display: "flex", borderBottom: "1px solid #C6C6C8", alignItems: "center", gap: "20px", padding: "16px 24px" }}>
          ...
          <Box sx={{ minWidth: "180px" }}>
            <Typography sx={{ color: "#666666", fontFamily: "Calibri", fontSize: "16px", mb: 0.5 }}>Receivable No. :</Typography>
            <Typography sx={{ fontSize: "28px", color: "#05595B", fontFamily: "Calibri", fontWeight: 400, lineHeight: 1 }}>{formData.receivableNo}</Typography>
          </Box>
          ...
          <Box sx={{ position: "relative", }}>
            <Account value={formData.account} onChange={handleAccountChange} options={customerList} disabled={fsmState === "view"} />
          </Box>
                ...
                <Table size="small" sx={{ minHeight: formData.items.length === 0 ? "256px" : "auto", tableLayout: "fixed", width: "100%" }}>
                  <TableHead sx={{ height: "32px" }}>
                    <TableRow sx={{ bgcolor: "#F2F2F2", }}>
                      <TableCell sx={{ padding: "6px 10px", width: "192px", minWidth: "192px", maxWidth: "192px", fontSize: "14px ", fontWeight: 700, borderBottom: "none", borderRight: "1px solid #EDEDED", fontFamily: "Calibri", color: "#343434", }}>Sale No.</TableCell>
                      ...
                      <>
                        {formData.items.map((item, index) => (
                          <TableRow key={index} sx={{ height: "38px" }}>
                            <TableCell sx={{ padding: "6px 10px", width: "192px", minWidth: "192px", maxWidth: "192px", borderRight: "1px solid #EDEDED", fontFamily: "Calibri", padding: "0 12px" }}>{item.saleNo}</TableCell>
                            ...
                            <TableCell align="right" sx={{ padding: "6px 10px", width: "192px", minWidth: "192px", maxWidth: "192px", borderRight: "1px solid #EDEDED", fontFamily: "Calibri", padding: 0 }}>
                              <NumericFormat
                                ...
                                onValueChange={(values) => {
                                  handleItemChange(index, "receive", values.value);
                                }}
                                sx={{
                                  width: "100%",
                        ...
                        <TableRow sx={{ bgcolor: "#FFFFFF", height: "38px" }}>
                          ...
                          <TableCell align="right" sx={{ width: "192px", minWidth: "192px", maxWidth: "192px", borderTop: "none", borderBottom: "none", fontSize: "16px ", fontWeight: 700, borderRight: "1px solid #EDEDED", fontFamily: "Calibri", color: "#666666", padding: "0 12px" }}>{formatNumberWithCommas(parseFloat(calculateTotal("receive") || 0).toFixed(2))}</TableCell>
                    ...
                    >
                      {bankThumbnailUrl ? (
                        ...
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                          ...
                          <path d="M14.333 7.91663H17.833" stroke="#343434" stroke-linecap="round" stroke-linejoin="round" />
                          <path d="M16.083 6.16663V9.66663" stroke="#343434" stroke-linecap="round" stroke-linejoin="round" />
                          <path d="M17.25 11.7083V16.0833C17.25 16.3928 17.1271 16.6895 16.9083 16.9083C16.6895 17.1271 16.3928 17.25 16.0833 17.25H7.91667C7.60725 17.25 7.3105 17.1271 7.09171 16.9083C6.87292 16.6895 6.75 16.3928 6.75 16.0833V7.91667C6.75 7.60725 6.87292 7.3105 7.09171 7.09171C7.3105 6.87292 7.60725 6.75 7.91667 6.75H12.2917" stroke="#343434" stroke-linecap="round" stroke-linejoin="round" />
                          <path d="M17.25 13.75L15.4498 11.9499C15.2311 11.7311 14.9344 11.6083 14.625 11.6083C14.3156 11.6083 14.0189 11.7311 13.8002 11.9499L8.5 17.25" stroke="#343434" stroke-linecap="round" stroke-linejoin="round" />
                          <path d="M10.2497 11.4167C10.894 11.4167 11.4163 10.8944 11.4163 10.25C11.4163 9.60571 10.894 9.08337 10.2497 9.08337C9.60534 9.08337 9.08301 9.60571 9.08301 10.25C9.08301 10.8944 9.60534 11.4167 10.2497 11.4167Z" stroke="#343434" stroke-linecap="round" stroke-linejoin="round" />
                        </svg>
                      )}
                    </Box>
                    <NumericFormat
