...
import { daybookOpenState, receivableFormDataState, receivableFSMState } from "../../../recoil/state/FinanceState";
...
const ReceivableDaybook = () => {
  const [open, setOpen] = useRecoilState(daybookOpenState);
  const [daybookData, setDaybookData] = useState([]);
  const [loading, setLoading] = useState(false);
  const [rowsPerPage, setRowsPerPage] = useState(10);
  const [selectedItemIds, setSelectedItemIds] = useState([]);
  const setFormData = useSetRecoilState(receivableFormDataState);
  const setFsmState = useSetRecoilState(receivableFSMState);
  ...
  const fetchDaybook = async () => {
    setLoading(true);
    try {
      const response = await apiRequest("GET", "/receivables");
      if (response) {
  ...
  const handleOk = async () => {
    ...
    setLoading(true);
    try {
      const response = await apiRequest("GET", `/receivables/${selectedItemIds[0]}`);
      if (response) {
        const data = response.data || response;

        setFormData({
          id: data._id || data.id,
          receivableNo: data.invoice_no || data.receivable_no || "",
        ...
        >
          ...
          >
            ...
            <Box sx={{ display: "flex", bgcolor: "#EDEDED", height: "42px", position: "sticky", top: 0, zIndex: 1, borderBottom: "1px solid #C6C6C8" }}>
              ...
              <Box sx={{ width: "140px", display: "flex", alignItems: "center", justifyContent: "center", gap: "4px" }}>
              ...
              <Box sx={{ width: "180px", display: "flex", alignItems: "center", justifyContent: "center" }}>
                <Typography sx={{ fontFamily: "Calibri", fontWeight: 700, fontSize: "16px", color: "#343434" }}>Receivable No.</Typography>
              </Box>
              ...
              <Box sx={{ width: "140px", display: "flex", alignItems: "center", justifyContent: "center" }}>
                <Typography sx={{ fontFamily: "Calibri", fontWeight: 700, fontSize: "16px", color: "#343434" }}>Receive</Typography>
              </Box>
              ...
              {daybookData.length > 0 ? (
                daybookData.map((row, index) => (
                  <Box key={index} sx={{ display: "flex", height: "42px", borderBottom: "1px solid #EDEDED", "&:hover": { bgcolor: "#F5F5F5" } }}>
                    ...
                    <Box sx={{ width: "180px", display: "flex", alignItems: "center", justifyContent: "center" }}>
                      <Typography sx={{ fontWeight: 400, fontFamily: "Calibri", fontSize: "16px", color: "#343434" }}>{row.invoice_no}</Typography>
                    </Box>
                    <Box sx={{ width: "250px", display: "flex", alignItems: "center", justifyContent: "center" }}>
...
export default ReceivableDaybook;
