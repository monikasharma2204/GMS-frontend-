...
if(e.target.checked){
    ...
    setMemoPendingIdListSelectedItem((prev)=>{
      // Prevent duplicate selections - only add if not already in the array
              ...
              {inventoryData?inventoryData.map((item, index) =>  
              {
                ...
                return (
                  item.inventory_item.map((inven_item,idx)=>{
                    accNum++
                    // Create a truly unique key using:
                    // 1. Parent invoice_no (unique per parent group)
                    // 2. Inner index (position within parent)
                    // 3. Sequential accNum (unique across all items)
                    // 4. Inventory item _id if available (database unique ID)
                    // This combination guarantees uniqueness even if items have similar properties
                    const uniqueKey = `${item.invoice_no || item._id || `parent_${index}`}_${idx}_${accNum}_${inven_item._id || `item_${idx}`}`;
                    const idx_checkbox = uniqueKey;
                    return (
                 ...
                 >
                  <Checkbox
                    ...
                    onChange={(e) => {
                      ...
                      handleCheckboxChange(e, idx_checkbox, inven_item);
                    }}
                  />
                   <Typography
                     sx={{
